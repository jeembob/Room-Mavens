To add to the card injector extension is item injection.

Item injection works by reapplying text data from the itemcards.json when it matches a specific href **and** when the text itself is not detected. Below are three examples:

# 1 Source Example

This is what we want to imitate. The below has text indicating for class "cost", "name", 
...
# Two scenarios that we want to inject dom elements into to look like the target example

Scenario 1: Right side up with no labels

<button class="image cell r1c0 svelte-1bk597j" style="grid-area: 1 / 2;"><svg viewBox="0 0 400 600" class="item svelte-zqd1cc selected"><image href="/public/images/byid/8639.image.webp" width="100%" height="100%" class="svelte-zqd1cc"></image></svg> <div class="svelte-1bk597j"></div></button>

Scenario 2: tapped (90 degree rotated to the right) with no labels

<button class="image cell r1c1 svelte-1bk597j" style="grid-area: 1 / 3;"><svg viewBox="0 0 600 400" class="item svelte-zqd1cc rotate"><image href="/public/images/byid/8652.image.webp" width="100%" height="100%" class="svelte-zqd1cc"></image></svg> <div class="svelte-1bk597j"></div></button>

# data has been extracted already

itemcards.json has that data

# what to look out for

The core issue is some pages have the data already in them, and some don't. We are looking for cases where they don't (i.e. scenario 1 and 2) and avoiding reinjection

# formatting

we should keep the same font and formatting as in the source example.
